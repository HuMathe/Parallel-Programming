...
public:
  ...
  // double *recv_buffer, *send_buffer;
...
public:
  ...
  hybrid_simulation(): grid_system() { init(); }
  hybrid_simulation(int num_row, int num_col)
    : grid_system(num_row, num_col) { init(); }
...
int rank, num_row, num_col, num_iter;
int main(int argc, char **argv)
{
  ...
  if(rank == 0)
    gui_init(&argc, argv, "MPI", 800, 800);

  hybrid_simulation heat_state(num_row, num_col);
